...
router.post('/:id/save',
    ...
    asyncHandler(async (req, res) => {
        validationErrorHandler(req, 'POST Assignment Status Report -')

        const {id} = req.params;
        const result = await query(
            `
                INSERT INTO assignment_status_report_snapshots
                (task_id,
                 project,
                 category,
                 assigned_employee,
...
router.get('/',
    ...
    requireRole(C_USER.ROLES.MANAGER),
    asyncHandler(async (req, res) => {
        validationErrorHandler(req, 'GET Assignment Status Reports -')
...
router.get('/history',
    ...
    asyncHandler(async (req, res) => {
        ...
        const result = await query(
            `
                SELECT *
                FROM assignment_status_report_snapshots
...
router.get('/:id',
    ...
    asyncHandler(async (req, res) => {
        validationErrorHandler(req, 'GET Assignment Status Report -')
