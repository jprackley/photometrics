 ...
 */
function buildOperationsReportData(projectRows = [], assignmentRows = [], taskRows = [], employeeRows = [], timeEntryRows = []) {
    ...
    const safeTimeEntries = Array.isArray(timeEntryRows) ? timeEntryRows : [];

    const trackedSecondsByTaskId = safeTimeEntries.reduce((map, entry) => {
        if (!entry?.taskId) return map;
        map.set(entry.taskId, (map.get(entry.taskId) || 0) + normalizeNumber(entry.totalSeconds));
        return map;
    }, new Map());

    const trackedSecondsByEmployeeId = safeTimeEntries.reduce((map, entry) => {
        if (!entry?.employeeId) return map;
        map.set(entry.employeeId, (map.get(entry.employeeId) || 0) + normalizeNumber(entry.totalSeconds));
        return map;
    }, new Map());

    const getTaskTrackedSeconds = (task) => {
        const liveEntrySeconds = trackedSecondsByTaskId.get(task.id) || trackedSecondsByTaskId.get(task.backendId) || 0;
        return liveEntrySeconds || normalizeNumber(task.trackedSeconds);
    };

    const getEmployeeTrackedSeconds = (employee, employeeTasks) => {
        const liveEntrySeconds = trackedSecondsByEmployeeId.get(employee.id) || trackedSecondsByEmployeeId.get(employee.userId) || trackedSecondsByEmployeeId.get(employee.backendId) || 0;
        if (liveEntrySeconds) return liveEntrySeconds;
        return employeeTasks.reduce((total, task) => total + getTaskTrackedSeconds(task), 0);
    };
    ...
    const totalTrackedSeconds = safeTasks.reduce((total, task) => total + getTaskTrackedSeconds(task), 0);
    ...
    const timeReportRows = safeTasks.map((task) => {
        const trackedSeconds = getTaskTrackedSeconds(task);
    ...
    const employeeReportRows = safeEmployees.map((employee) => {
        ...
        const employeeTrackedSeconds = getEmployeeTrackedSeconds(employee, employeeTasks);
